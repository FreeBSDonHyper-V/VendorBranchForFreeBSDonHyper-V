...
#define KVP_CONFIG_LOC	"/etc/hyperv/pool/"
				 ...
				 struct hv_kvp_ipaddr_value *buffer)
{
	 ...
	 * .
	 */
	/* Scripts are stored in /etc/hyperv/scripts/ directory */
	sprintf(cmd, "%s",  "sh /etc/hyperv/scripts/hv_get_dns_info");
	/*
	...
	sprintf(cmd, "%s %s", "sh /etc/hyperv/scripts/hv_get_dhcp_info", if_name);
...
static int kvp_set_ip_info(char *if_name, struct hv_kvp_ipaddr_value *new_val)
{
	...
	*/
	snprintf(if_file, sizeof(if_file), "%s%s","/etc/hyperv/","hv_set_test");
...
setval_done:
	...
	snprintf(cmd, sizeof(cmd), "%s %s", "sh /etc/hyperv/scripts/hv_set_ifconfig", if_file);
	system(cmd);
	return 0;
